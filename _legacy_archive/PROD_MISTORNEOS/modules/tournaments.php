...
if ($action === 'delete' && $id) {
    try {
        ...
        if ($result && $stmt->rowCount() > 0) {
            header('Location: index.php?page=tournaments&success=' . urlencode('Torneo "' . $tournament_to_delete['nombre'] . '" eliminado exitosamente'));
        } else {
            throw new Exception('No se pudo eliminar el torneo');
        }
    } catch (Exception $e) {
        header('Location: index.php?page=tournaments&error=' . urlencode('Error al eliminar: ' . $e->getMessage()));
    }
    exit;
}
                                                ...
                                                <div class="col-md-6">
                                                    <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                                                        ...
                                                        <?php if ($puede_editar): ?>
                                                            ...
                                                            <?php if (!$es_iniciado): ?><a href="index.php?page=tournaments&action=delete&id=<?= $item['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('¿Eliminar este torneo?')"><i class="fas fa-trash me-1"></i>Eliminar</a><?php endif; ?>
                                            ...
                                            <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                                                ...
                                                <?php if ($puede_editar): ?>
                                                    ...
                                                    <a href="index.php?page=tournaments&action=delete&id=<?= $item['id'] ?>" 
                                                       ...
                                                       onclick="return confirm('¿Está seguro de eliminar el torneo \'<?= htmlspecialchars($item['nombre']) ?>\'?')"
                                                       title="Eliminar torneo (solo torneos futuros sin rondas)">
...
<?php elseif ($action === 'new' || $action === 'edit'): ?>
    ...
    <div class="d-flex justify-content-center align-items-center min-vh-100 py-5" style="background: linear-gradient(135deg, #1a365d 0%, #2d3748 100%);">
        <div class="card shadow-lg" style="max-width: 60%; width: 60%;">
            ...
            <div class="card-body">
            <form method="POST" action="<?= $action === 'edit' ? 'tournament_update.php' : 'tournament_save.php' ?>" enctype="multipart/form-data">
                <?= CSRF::input(); ?>
                <?php if ($action === 'edit'): ?>
                            ...
                            <select class="form-select" id="club_responsable" name="club_responsable" <?= $action === 'new' ? 'required' : '' ?>>
                                ...
                                <?php foreach ($organizaciones_list as $org): ?>
                                    <option value="<?= (int)$org['id'] ?>"
                                            <?= ($action === 'edit' && ($tournament['club_responsable'] ?? 0) == $org['id']) ? 'selected' : '' ?>>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                try {
                                    ...
                                    if ($user_role === 'admin_club' && $has_owner_column) {
                                        ...
                                        ");
                                        $stmt_cuentas->execute([(int)$current_user['id']]);
                                    } else {
                                        $stmt_cuentas = $pdo_cuentas->query("
                                            SELECT id, banco, numero_cuenta, nombre_propietario, tipo_cuenta, telefono_afiliado
                                            FROM cuentas_bancarias
                                            WHERE estatus = 1
                                            ORDER BY banco, nombre_propietario ASC
                                ...
                                <small class="form-text text-muted">
                                    <?php 
                                    $return_torneo_url = 'index.php?page=cuentas_bancarias&action=new&return_torneo=1&torneo_action=' . ($action === 'edit' ? 'edit' : 'new');
