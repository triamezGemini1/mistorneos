...
/**
 * LandingDataService - Servicio ├║nico para datos de torneos en el landing p├║blico.
 ...
 */
final class LandingDataService
{
    private PDO $pdo;
    ...
    /**
     * Obtiene los pr├│ximos eventos (fechator >= CURDATE(), estatus = 1).
     ...
     *
     * @param int $limit
     * @param int|null $anio Filtro por a├▒o (opcional)
    ...
    /**
     * Obtiene pr├│ximos eventos filtrados por entidad (organizaciones con esa entidad).
     *
    ...
    public function getProximosEventosPorEntidad(int $entidad_id, int $limit = 12): array
    {
        ...
        $sql = "
            ...
            LEFT JOIN organizaciones o ON t.club_responsable = o.id AND o.estatus = 1
            LEFT JOIN usuarios u ON o.admin_user_id = u.id AND u.role = 'admin_club'
            LEFT JOIN entidades e ON COALESCE(o.entidad, t.entidad) = e.id
            WHERE t.estatus = 1 AND t.fechator >= CURDATE()
    ...
    /**
     * Obtiene pr├│ximos eventos filtrados por IDs de organizaci├│n (club_responsable IN org_ids).
     *
    ...
    /**
     * Obtiene IDs de organizaci├│n para una entidad (orgs cuya entidad = X).
     */
    ...
    /**
     * Obtiene IDs de organizaci├│n para el club del usuario (club.organizacion_id).
     */
    ...
    {
        foreach (['entidades', 'entidad'] as $tbl) {
            try {
                $cols = $this->pdo->query('SHOW COLUMNS FROM `' . str_replace('`', '', $tbl) . '`')->fetchAll(PDO::FETCH_ASSOC);
                foreach ($cols as $col) {
     ...
     * Las entidades sin organizaciones se mostrar├ín inactivas (no seleccionables).
     *
        ...
        try {
            ...
            $fallback = $this->pdo->query("
                SELECT
                    o.entidad AS id,
                    COALESCE(
                        (SELECT e2.nombre FROM entidades e2 WHERE e2.{$col} = o.entidad LIMIT 1),
    ...
    /**
     * Obtiene clubes de una organizaci├│n (para flujo de registro).
     *
     * @param int $org_id
     * @return list<array>
     */
