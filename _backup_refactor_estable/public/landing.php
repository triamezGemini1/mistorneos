...
/**
 * Landing público (recuperado desde branch main, adaptado a Connection + AuthHelper).
 */
...
require_once $root . '/app/Helpers/AppBaseUrl.php';
require_once $root . '/app/Helpers/UrlHelper.php';
require_once $root . '/app/Core/LandingDataService.php';
require_once __DIR__ . '/landing_meta.php';

$script = $_SERVER['SCRIPT_NAME'] ?? '';
$publicPrefix = str_contains($script, '/public/') ? '' : 'public/';
$GLOBALS['publicPrefix'] = $publicPrefix;
...
try {
    ...
    echo '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Servicio no disponible</title></head><body><p>No hay conexión a la base de datos.</p></body></html>';
    exit;
}

$user = AuthHelper::isLoggedIn() ? AuthHelper::currentUser() : null;
$landingService = new LandingDataService($pdo);

// Eventos realizados (LandingDataService) - l├¡mite amplio para mostrar torneos antiguos (ej. torneo id=1)
...
if ($entidad_filtro > 0 && empty($eventos_mi_entidad)) {
    foreach (['entidades', 'entidad'] as $tblEnt) {
        try {
            $stmt = $pdo->prepare('SELECT nombre FROM `' . str_replace('`', '', $tblEnt) . '` WHERE id = ? LIMIT 1');
            $stmt->execute([$entidad_filtro]);
            $ent_data = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($ent_data) {
                $entidad_nombre_usuario = (string) ($ent_data['nombre'] ?? '');
...
// Funci├│n helper para obtener URL de afiche (compatible con view_tournament_file.php)
...
// Funci├│n helper para obtener URL del logo de la organizaci├│n
...
// Funci├│n helper para formatear modalidad y clase
...
// ├ìndice de eventos por fecha (Y-m-d) para el calendario interactivo (con URLs para tarjetas)
$eventos_por_fecha = [];
...
<head>
    ...
    <!-- SEO Meta Tags - IMPORTANTE: No usar noindex para permitir indexaci├│n -->
    ...
    <link rel="stylesheet" href="<?= htmlspecialchars($publicPrefix . 'assets/dist/output.css', ENT_QUOTES, 'UTF-8') ?>">
    ...
    <style>
        ...
        /* Logo de organizaci├│n en publicaciones: 60% del contenedor, sin distorsi├│n en todos los dispositivos */
        .landing-logo-org {
            max-height: 60%;
            max-width: 60%;
            width: auto;
        ...
        /* Calendario anual - a├▒o completo en un bloque sin scroll */
        ...
        /* Colores por estado de fecha: rojo=pasados, verde=pr├│ximas 24h, azul=pr├│ximas */
        ...
        /* Mes ampliado: nombres de d├¡as en una fila, fechas alineadas debajo de cada columna */
        #cal-mes-header,
        #grid-mes-ampliado {
...
<?php include_once __DIR__ . '/components/hero.php'; ?>
    ...
    <section id="registro" class="py-16 md:py-24 bg-white">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold text-primary-700 mb-4">├Ünete a Nuestra Comunidad</h2>
                    ...
                    <div class="relative bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full">
                        <div class="text-center mb-6">
                            ...
                            <p class="text-white/90 mb-6">
                                Para jugadores que desean participar en torneos de domin├│ en cualquier ubicaci├│n.
                            </p>
                        </div>
                        ...
                        <ul class="space-y-3 mb-6 text-left">
                            ...
                            <li class="flex items-center">
                                <i class="fas fa-check-circle mr-3 text-white/90"></i>
                                <span>Sin restricciones geogr├íficas</span>
                            </li>
                            <li class="flex items-center">
                                <i class="fas fa-check-circle mr-3 text-white/90"></i>
                                <span>Credencial digital ├║nica</span>
                            </li>
                        </ul>
                        
                        <div class="space-y-2">
                            <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>afiliacion.php" class="block w-full px-6 py-3 bg-white text-emerald-600 font-semibold rounded-xl hover:bg-gray-100 transition-all duration-200 text-center shadow-lg">
                                <i class="fas fa-building mr-2"></i>Seleccionar Club y Registrarme
                            </a>
                            <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php#mn-registro-padron" class="block text-center text-white/80 hover:text-white text-sm transition-colors">
                ...
                <div class="group relative">
                    ...
                    <div class="relative bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full">
                        <div class="text-center mb-6">
                            ...
                            <h3 class="text-2xl font-bold mb-3">Solicitud de Afiliaci├│n</h3>
                            <p class="text-white/90 mb-6">
                        ...
                        <a href="afiliacion.php" class="block w-full px-6 py-3 bg-white text-rose-600 font-semibold rounded-xl hover:bg-gray-100 transition-all duration-200 text-center shadow-lg">
                            <i class="fas fa-paper-plane mr-2"></i>Solicitar Afiliaci├│n
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center mt-10">
                <p class="text-gray-600 flex items-center justify-center">
                    ...
                    Las solicitudes de afiliaci├│n ser├ín revisadas por el administrador del sistema.
                </p>
            </div>
        </div>
    </section>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                ...
                <p class="text-lg md:text-xl text-white/90 max-w-3xl mx-auto">
                    Inscr├¡bete desde tu dispositivo m├│vil en estos eventos. Abierto a jugadores de todas las entidades.
                </p>
            </div>
                ...
                <?php foreach ($eventos_masivos as $evento): ?>
                    ...
                    <div class="bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-300 overflow-hidden border-2 border-white/20 hover:border-yellow-400 transform hover:-translate-y-2 text-center">
                        ...
                        <div class="p-6 text-center flex flex-col items-center">
                            ...
                            <a href="resultado_torneo.php?torneo_id=<?= (int)$evento['id'] ?>" 
                               class="block w-full px-4 py-2 mb-2 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-lg border border-white/40 transition-all text-center">
                                <i class="fas fa-info-circle mr-2"></i>Ver informaci├│n del torneo
                            </a>
                            <?php 
                            ...
                            <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php?invitacion=1&torneo=<?= (int) $evento['id'] ?>#mn-registro-padron" 
                            ...
                            <div class="bg-blue-400/20 rounded-lg p-3 border border-white/40">
                                <p class="text-xs text-purple-900 text-center mb-0">Inscripci├│n deshabilitada el d├¡a del torneo. Presentarse al evento.</p>
                            </div>
                            <?php else: ?>
                            <div class="bg-yellow-400/20 rounded-lg p-3 mb-3 border border-yellow-400/50">
                                <p class="text-xs text-purple-900 text-center mb-2">
                                    <i class="fas fa-info-circle mr-1"></i>Inscripci├│n en sitio. Contacta al organizador.
                                </p>
                                <?php if ($tel_contacto): ?>
                                <a href="tel:<?= htmlspecialchars(preg_replace('/[^0-9+]/', '', $tel_contacto)) ?>" 
                                   class="block w-full px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all text-center shadow-lg">
                                    <i class="fas fa-phone mr-2"></i>Contactar administraci├│n
                                </a>
                                <?php else: ?>
    ...
    <!-- Eventos con Inscripci├│n en L├¡nea (C├│digo 2) -->
    ...
    <section id="eventos-inscripcion-linea" class="py-16 md:py-24 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 text-white">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
                    <i class="fas fa-globe mr-3 text-yellow-400"></i>Inscripci├│n en L├¡nea
                </h2>
                <p class="text-lg md:text-xl text-white/90 max-w-3xl mx-auto">
                    Inscr├¡bete directamente en estos eventos. 
                </p>
            </div>
                        ...
                        <div class="p-6 text-center">
                            ...
                            <?php if ($evento['costo'] > 0): ?>
                                <div class="bg-blue-400/20 rounded-lg p-3 mb-4 border border-blue-400/50">
                                    <p class="text-xs text-blue-200 text-center">
                                        ...
                                        <strong>Costo:</strong> $<?= number_format($evento['costo'], 2) ?> - Puedes pagar despu├®s de inscribirte
                                    </p>
                                </div>
                            <?php endif; ?>
                            <a href="resultado_torneo.php?torneo_id=<?= (int)$evento['id'] ?>" 
                            ...
                            <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php?invitacion=1&torneo=<?= (int) $evento['id'] ?>#mn-registro-padron" 
                               class="block w-full px-4 py-3 bg-gradient-to-r from-yellow-400 to-orange-500 text-blue-900 font-bold rounded-lg hover:from-yellow-500 hover:to-orange-600 transition-all text-center shadow-lg hover:shadow-xl transform hover:scale-105">
                                <i class="fas fa-sign-in-alt mr-2"></i>Inscr├¡bete Ahora
                            </a>
                            ...
                            <div class="bg-blue-400/30 rounded-lg p-3 border border-white/40">
                                <p class="text-xs text-white text-center mb-0">
                                    <i class="fas fa-calendar-day mr-1"></i>Inscripci├│n en l├¡nea deshabilitada el d├¡a del torneo. Presentarse al evento para participar.
                                </p>
                            </div>
                            <?php else: ?>
    ...
    <!-- Eventos Privados Section (C├│digo 4 - Solo visualizaci├│n) -->
    ...
    <!-- Eventos por Entidad Section (P├║blico) -->
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8 mb-12">
                <?php foreach ($entidades_con_eventos as $ent): ?>
                    <a href="index.php?entidad=<?= $ent['id'] ?>#eventos-entidad" 
                        ...
                        <div class="p-6">
                            ...
                            <div class="space-y-2 mt-4">
                                ...
                                <div class="flex items-center justify-between p-2 bg-green-50 rounded-lg">
                                    <span class="text-sm text-gray-600 flex items-center">
                                        <i class="fas fa-calendar-check mr-2 text-green-500"></i>Pr├│ximos
                                    </span>
            ...
            <?php if ($entidad_seleccionada > 0): ?>
                <div class="mb-8 text-center">
                    <a href="index.php#eventos-entidad" class="inline-flex items-center px-6 py-3 bg-gray-500 text-white font-semibold rounded-xl hover:bg-gray-600 transition-all shadow-lg hover:shadow-xl mb-6">
            ...
            <?php if (!empty($eventos_mi_entidad)): ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    <?php foreach ($eventos_mi_entidad as $evento): ?>
                        ...
                        // Obtener informaci├│n del administrador y clubes afiliados
                        $admin_info = null;
                        $clubes_afiliados = [];
                        if (!empty($evento['club_id'])) {
                            try {
                                ...
                                error_log("Error obteniendo informaci├│n del club: " . $e->getMessage());
                            }
                        }
                        ?>
                            ...
                            <div class="p-6 text-center">
                                ...
                                <div class="bg-gray-50 rounded-xl p-4 mb-4 text-left">
                                    ...
                                    <?php if ($evento['club_telefono']): ?>
                                        <p class="text-sm text-gray-700 mb-1">
                                            <strong><i class="fas fa-phone mr-2 text-primary-500"></i>Tel├®fono:</strong> <?= htmlspecialchars($evento['club_telefono']) ?>
                                        </p>
                                    <?php endif; ?>
                                            ...
                                            <button type="button" 
                                                ...
                                                <span>
                                                    ...
                                                    <?php elseif ($admin_info): ?>
                                                        Ver Informaci├│n del Administrador
                                ...
                                <a href="resultado_torneo.php?torneo_id=<?= (int)$evento['id'] ?>" class="block w-full px-4 py-2 bg-primary-500 text-white font-semibold rounded-lg hover:bg-primary-600 transition-all text-center mb-2">
                                    <i class="fas fa-info-circle mr-2"></i>Ver informaci├│n del torneo
                                </a>
                                <?php 
                                $permite_online = (int)($evento['permite_inscripcion_linea'] ?? 1) === 1; 
                                ...
                                <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php?invitacion=1&torneo=<?= (int) $evento['id'] ?>#mn-registro-padron" class="block w-full px-4 py-2 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600 transition-all text-center mb-2">
                                    <i class="fas fa-sign-in-alt mr-2"></i>Inscribirme
                                </a>
                                <?php elseif ($permite_online && $es_hoy_ent): ?>
                                <p class="text-xs text-gray-500 text-center mb-2">Inscripci├│n deshabilitada el d├¡a del torneo.</p>
                                ...
                                <?php endif; ?>
                                <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php#buscar" class="block w-full px-4 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition-all text-center">
            ...
            <?php elseif ($entidad_seleccionada > 0): ?>
                <div class="text-center py-12 bg-white rounded-2xl shadow-lg">
                    ...
                    <p class="text-gray-500 text-sm">Intenta seleccionar otra entidad o consulta los eventos generales m├ís abajo</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
    ...
    <section id="eventos" class="py-16 md:py-24 bg-gray-50">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-16">
                <div class="text-center mb-12">
                    <h2 class="text-3xl md:text-4xl font-bold text-primary-700 mb-4">
                        <i class="fas fa-calendar-check mr-3 text-accent"></i>Pr├│ximos Eventos
                    </h2>
            ...
            <div>
                <div class="text-center mb-12">
                    ...
                    <p class="text-lg text-gray-600">Revisa los resultados y fotograf├¡as de eventos pasados</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    <?php foreach ($eventos_realizados as $evento): ?>
                        <?php 
                        // Asegurar que total_fotos est├® definido antes de usarlo
                ...
                <div class="text-center mt-8">
                    <a href="<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>resultados.php" class="inline-flex items-center px-6 py-3 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 transition-all shadow-lg hover:shadow-xl">
                        <i class="fas fa-history mr-2"></i>Ver Hist├│rico Completo
                    </a>
                </div>
            </div>
            <?php endif; ?>
    ...
    <!-- Calendario Section - A├▒o completo en un bloque, sin scroll -->
    <section id="calendario" class="py-3 min-h-[85vh]">
        <div class="container mx-auto px-3 sm:px-4">
            ...
            <!-- VISTA 1: A├▒o completo en un bloque - Enero, Febrero, Marzo... sin scroll -->
            <div id="vista-anual" class="cal-vista">
                ...
                <div class="cal-contenedor-anual">
                    <div id="grid-anual">
                        <!-- 12 meses: Enero, Febrero, Marzo... con d├¡as y fechas -->
                    </div>
                </div>
            </div>
            ...
            <div id="vista-mes" class="cal-vista hidden">
                ...
                <div class="bg-white rounded-lg border border-slate-200 shadow overflow-hidden max-w-5xl mx-auto cal-mes-ampliado">
                    ...
                    <div class="p-4">
                        <!-- Fila ├║nica de nombres de d├¡as: Do, Lu, Ma, Mi, Ju, Vi, Sa -->
                        <div id="cal-mes-header" class="grid grid-cols-7 gap-2 mb-2">
                            <!-- Se rellena por JS -->
                        </div>
                        <!-- Grid de fechas alineadas debajo de cada columna de d├¡a -->
                ...
                <!-- Secci├│n torneos del d├¡a: oculta cuadr├¡cula, muestra tarjetas (mismo formato que publicaciones en landing) -->
                <div id="seccion-eventos-dia" class="hidden rounded-2xl overflow-hidden bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 text-white p-6 shadow-xl">
                    <div class="flex items-center justify-between mb-6">
                        <h4 id="eventos-dia-titulo" class="text-xl font-bold text-white">Torneos del d├¡a</h4>
    ...
    (function() {
        ...
        const baseUrl = '<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>';
        ...
        /** Domingo = 0, Lunes = 1, ... S├íbado = 6 (para calendario Do, Lu, Ma, Mi, Ju, Vi, Sa) */
        ...
        function mostrarVistaMes(fechaConEventos) {
            ...
            // Fila ├║nica: Do, Lu, Ma, Mi, Ju, Vi, Sa
            let headerHtml = '';
            ...
            // Fechas alineadas debajo de cada columna de d├¡a
            let html = '';
        ...
        function renderTarjetaEvento(ev) {
            ...
            html += '</div>';
            html += '<a href="' + baseUrl + 'resultado_torneo.php?torneo_id=' + ev.id + '" class="block w-full px-4 py-2 mb-2 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-lg border border-white/40 transition-all text-center"><i class="fas fa-info-circle mr-2"></i>Ver informaci├│n del torneo</a>';
            if (esPasado) {
                html += '<a href="' + baseUrl + 'resultado_torneo.php?torneo_id=' + ev.id + '" class="block w-full px-4 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-bold rounded-lg hover:from-emerald-600 hover:to-emerald-700 transition-all text-center shadow-lg"><i class="fas fa-trophy mr-2"></i>Ver Resultados</a>';
            } else if (permiteOnline) {
                const urlInsc = '<?= htmlspecialchars($publicPrefix, ENT_QUOTES, 'UTF-8') ?>index.php?invitacion=1&torneo=' + ev.id + '#mn-registro-padron';
                ...
                html += '<div class="bg-yellow-400/20 rounded-lg p-3 mb-3 border border-yellow-400/50"><p class="text-xs text-purple-900 text-center mb-2"><i class="fas fa-info-circle mr-1"></i>Inscripci├│n en sitio. Contacta al organizador.</p>';
                if (telContacto) html += '<a href="tel:' + telContacto.replace(/\D/g,'') + '" class="block w-full px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all text-center shadow-lg"><i class="fas fa-phone mr-2"></i>Contactar administraci├│n</a>';
    ...
    <section id="galeria" class="py-16 md:py-24 bg-gray-50">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-3xl md:text-4xl font-bold text-primary-700 mb-4">
                    <i class="fas fa-images mr-3 text-accent"></i>Galer├¡a de Torneos
                </h2>
                <p class="text-lg text-gray-600">Momentos destacados de nuestros eventos</p>
            </div>

            <?php
            // Obtener 4 fotos del ├║ltimo torneo
            $fotos_ultimo_torneo = [];
            ...
            try {
                ...
                if ($club_photos_exists) {
                    // Obtener el ├║ltimo torneo con fotos
                    $stmt = $pdo->query("
                        SELECT 
                            t.id as torneo_id,
                ...
                error_log("Error obteniendo fotos del ├║ltimo torneo: " . $e->getMessage());
            }
            ?>
            ...
            <?php else: ?>
                <div class="text-center py-12 bg-white rounded-2xl shadow-lg">
                    ...
                    <p class="text-gray-600 text-lg mb-4">Pr├│ximamente: Galer├¡a de fotos de torneos</p>
                    <a href="<?= htmlspecialchars(app_base_url() . '/public/galeria_fotos.php') ?>" class="inline-block bg-primary-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-primary-600 transition-all">
                        <i class="fas fa-images mr-2"></i>Ver Galer├¡a
                    </a>
                </div>
            <?php endif; ?>
        </div>
    ...
    <script>
        ...
        document.querySelectorAll('a[href^="#"]').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                ...
                if (target) {
                    ...
                    // Cerrar men├║ m├│vil si est├í abierto
                    document.getElementById('mobile-menu')?.classList.add('hidden');
                }
            });
        });

        // Funci├│n para ver fotos del evento
                ...
                <div class="bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
                    <div class="flex items-center justify-between p-6 border-b border-gray-200">
                        <h5 class="text-xl font-bold text-gray-900 flex items-center">
                            <i class="fas fa-images mr-3 text-primary-500"></i>Fotograf├¡as - ${torneoNombre}
                        </h5>
                    ...
                    <div class="p-6 overflow-y-auto max-h-[calc(90vh-100px)]">
                        <div id="galeria-loading" class="text-center py-12">
                            ...
                            <p class="text-gray-600">Cargando fotograf├¡as...</p>
                        </div>
            ...
            try {
                ...
                if (data.success && data.fotos && data.fotos.length > 0) {
                    ...
                    loadingDiv.innerHTML = `
                        ...
                        <p class="text-gray-600">No hay fotograf├¡as disponibles para este evento.</p>
                    `;
                }
            } catch (error) {
                ...
                loadingDiv.innerHTML = `
                    ...
                    <p class="text-gray-600">Error al cargar las fotograf├¡as.</p>
                `;
            }
        }
        
        // Funci├│n para ver imagen en tama├▒o completo
        ...
        // Funci├│n para ver imagen completa de la galer├¡a
        ...
        // Funci├│n para mostrar/ocultar informaci├│n del administrador y clubes afiliados
...
?>
<script src="<?= htmlspecialchars($publicPrefix . 'assets/js/auth-modal.js', ENT_QUOTES, 'UTF-8') ?>" defer></script>
</body>
</html>
