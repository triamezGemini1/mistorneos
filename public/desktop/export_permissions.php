 ...
 * Usa el mismo endpoint que export_to_web (sync_api.php) enviando solo los registros de staff modificados.
 */
...
if ($pushUrl === '' || $apiKey === '') {
    ...
    echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Exportar permisos</title></head><body><p>' . htmlspecialchars($msg) . '</p><a href="gestion_staff.php">Volver</a></body></html>';
    exit;
}
...
if (empty($rows)) {
    $msg = 'No hay cambios de permisos pendientes de subir.';
    if (php_sapi_name() === 'cli') {
        echo $msg . PHP_EOL;
        exit(0);
    }
    header('Location: gestion_staff.php?export=0');
    exit;
}
...
$total = count($rows);
$message = "Permisos exportados: {$total} administrador(es) actualizado(s) en la web.";

if ($isCli) {
    echo $message . PHP_EOL;
    exit(0);
}
header('Location: gestion_staff.php?export=1&n=' . $total);
exit;
