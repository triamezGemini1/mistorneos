...
$pageTitle = 'Reporte equipos (detallado)';
$desktopActive = 'panel';
require_once __DIR__ . '/desktop_layout.php';
?>
<div class="container-fluid py-3">
    <h2 class="h4 mb-3"><i class="fas fa-th-list text-primary me-2"></i>Reporte equipos (detallado)</h2>
    <p class="text-muted">Detalle de resultados por equipo y jugador. Solo aplica a torneos por equipos.</p>
    <div class="card border-0 shadow-sm" style="border-radius: 12px; border: 1px solid #E9ECEF;">
        <div class="card-body">
            <form method="get" action="reporte_equipos_detallado.php" class="row g-3 mb-3">
                <div class="col-md-6">
            ...
            <?php if ($torneo): ?>
            <p class="mb-0 text-muted small"><strong><?= htmlspecialchars($torneo['nombre'] ?? '') ?></strong> — Rondas: <?= (int)($torneo['rondas'] ?? 0) ?>. Contenido del reporte detallado por equipos se cargará aquí.</p>
            <?php else: ?>
            <p class="text-muted small mb-0">Seleccione un torneo por equipos para ver el reporte detallado.</p>
            <?php endif; ?>
