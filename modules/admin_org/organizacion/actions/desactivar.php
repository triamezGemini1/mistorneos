...
try {
    ...
    if ($return_to === 'organizaciones' && $entidad_id > 0) {
        header('Location: ' . $base . 'index.php?page=organizaciones&entidad_id=' . $entidad_id . '&success=' . urlencode('Organización desactivada'));
    } else {
        header('Location: ' . $base . 'index.php?page=organizaciones&success=' . urlencode('Organización desactivada'));
    }
    exit;
} catch (Exception $e) {
    header('Location: ' . $base . 'index.php?page=organizaciones&error=' . urlencode('Error al desactivar: ' . $e->getMessage()));
    exit;
}
