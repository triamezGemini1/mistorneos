...
require_once __DIR__ . '/TeamPerformanceHandler.php';
...
final class TournamentStatusHandler
{
    ...
    public static function getTournamentSummary(int $torneoId): array
    {
        ...
        if ($ultima_ronda > 0) {
            ...
            $ultima_ronda_tiene_resultados = $mesaServicePanel->rondaTieneResultadosEnMesas($torneoId, $ultima_ronda);
        }
        ...
        if ((int) ($torneo['modalidad'] ?? 0) === 3) {
            $rankingEquipos = TeamPerformanceHandler::getRankingPorEquipos($torneoId, 'puntos');
            $total_equipos = count($rankingEquipos);
            foreach ($rankingEquipos as $eq) {
                $total_jugadores_inscritos += (int) ($eq['total_jugadores'] ?? 0);
            }
        }
