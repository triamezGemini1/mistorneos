...
try {
    ...
    // entidad: debe ser la misma del admin que edita (no puede ser 0, no aceptar desde POST)
    $entidad = 0;
    if ($tiene_entidad_col) {
        $org_id = $club_responsable ?? $torneo_actual['club_responsable'] ?? null;
        $entidad = (int)($torneo_actual['entidad'] ?? 0);
        if ($entidad <= 0 || $is_admin_general) {
            // Obtener entidad de la organización o del usuario
            if ($org_id) {
                $stmt_ent = DB::pdo()->prepare("SELECT entidad FROM organizaciones WHERE id = ?");
                $stmt_ent->execute([$org_id]);
                $entidad_nueva = (int)$stmt_ent->fetchColumn();
                if ($entidad_nueva > 0) {
                    $entidad = $entidad_nueva;
                }
            }
            if ($entidad <= 0) {
                $entidad = (int)($current_user['entidad'] ?? 0);
            }
        }
        if ($entidad <= 0) {
            throw new Exception('La entidad del administrador no está definida. No se puede actualizar el torneo.');
        }
        // Validar que la entidad del torneo coincida con la del admin (salvo admin_general)
        if (!$is_admin_general) {
            $entidad_admin = 0;
            if ($org_id) {
                $stmt_ea = DB::pdo()->prepare("SELECT entidad FROM organizaciones WHERE id = ?");
                $stmt_ea->execute([$org_id]);
                $entidad_admin = (int)$stmt_ea->fetchColumn();
            }
            if ($entidad_admin <= 0) {
                $entidad_admin = (int)($current_user['entidad'] ?? 0);
            }
            if ($entidad_admin > 0 && $entidad !== $entidad_admin) {
                throw new Exception('No puede modificar este torneo: la entidad del torneo no coincide con la suya.');
            }
        }
    }
