...
try {
    ...
    // entidad: debe ser la misma del admin que crea el torneo (no puede ser 0, no aceptar desde POST)
    $entidad = 0;
    if ($organizacion_id) {
    ...
    if ($entidad <= 0) {
        ...
        throw new Exception('No se puede registrar el torneo sin entidad válida. La organización debe tener entidad definida.');
    }
    ...
    // Redirigir con �xito
    $redirect_url = app_base_url() . '/public/index.php?page=tournaments&success=' . urlencode('Torneo creado exitosamente');
    header('Location: ' . $redirect_url);
    exit;
    
} catch (Exception $e) {
    // Redirigir con error
    $redirect_url = app_base_url() . '/public/index.php?page=tournaments&action=new&error=' . urlencode($e->getMessage());
    header('Location: ' . $redirect_url);
    exit;
}
