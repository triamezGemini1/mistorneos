...
function obtenerTopJugadoresPorClub($pdo, $torneo_id, $topN) {
    ...
    // Ordenar estadísticas por mejor posición, luego por puntos ranking
    usort($estadisticas, function($a, $b) {
        if ($a['mejor_posicion'] != $b['mejor_posicion']) {
            if ($a['mejor_posicion'] == 0) return 1;
            if ($b['mejor_posicion'] == 0) return -1;
            return $a['mejor_posicion'] <=> $b['mejor_posicion'];
        }
...
try {
    ...
    // Ordenar clubes por mejor posición, luego por puntos ranking
