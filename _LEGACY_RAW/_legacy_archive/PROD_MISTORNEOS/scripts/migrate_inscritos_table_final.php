<?php
/**
 * Script de migración para crear tabla inscritos
 * 
 * Este script crea la tabla inscritos que se relaciona con partiresul
 * durante todo el proceso del torneo.
 */

require_once __DIR__ . '/../config/bootstrap.php';
require_once __DIR__ . '/../config/db.php';

echo "=== MIGRACIÓN: Creación de tabla inscritos ===\n\n";
...
try {
    ...
    // Leer el archivo SQL
    $sql_file = __DIR__ . '/../sql/migrate_inscritos_table_final.sql';
    ...
    $statements = array_filter(
        ...
        function($stmt) {
            ...
            return !empty($stmt_clean) && 
                   ...
                   !preg_match('/^RELACIÓN CON/i', $stmt_clean);
        }
    );
    ...
    foreach ($statements as $index => $statement) {
        ...
        if (preg_match('/^--|^\/\*|\*\/$|NOTAS IMPORTANTES|RELACIÓN CON/i', $statement)) {
            continue;
        }
    ...
    // Verificar índices
    echo "\nVerificando índices...\n";
    $stmt = $pdo->query("SHOW INDEX FROM inscritos");
    ...
    // Verificar relación con partiresul
    echo "\nVerificando compatibilidad con partiresul...\n";
    $stmt = $pdo->query("SHOW TABLES LIKE 'partiresul'");
    if ($stmt->rowCount() > 0) {
        echo "✓ Tabla partiresul existe\n";
        
        // Verificar campos comunes
        $stmt = $pdo->query("DESCRIBE partiresul");
        $partiresul_columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $partiresul_fields = array_column($partiresul_columns, 'Field');
        
        $required_fields = ['id_usuario', 'id_torneo'];
        $missing_fields = [];
        
        foreach ($required_fields as $field) {
            if (in_array($field, $partiresul_fields)) {
                echo "  ✓ Campo común '{$field}' existe en partiresul\n";
            } else {
                $missing_fields[] = $field;
                echo "  ⚠ Campo común '{$field}' NO existe en partiresul\n";
            }
        }
        
        if (empty($missing_fields)) {
            echo "\n✓ Las tablas inscritos y partiresul son compatibles\n";
        } else {
            echo "\n⚠ Faltan campos en partiresul para la relación: " . implode(', ', $missing_fields) . "\n";
        }
    } else {
        echo "⚠ Tabla partiresul no existe aún. Ejecuta: php scripts/migrate_partiresul_table.php\n";
    }
    
    echo "\n=== MIGRACIÓN COMPLETADA ===\n";
    echo "\nLa tabla inscritos está lista para recibir inscripciones.\n";
    echo "Se relacionará con partiresul mediante id_usuario e id_torneo.\n";
