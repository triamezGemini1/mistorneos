...
// Contar actas pendientes de verificación (admin_club y admin_general)
$actas_pendientes_count = 0;
if (in_array($user['role'], ['admin_club', 'admin_general'], true)) {
    try {
  ...
  <div class="d-flex" id="wrapper">
    ...
    <nav id="sidebar" class="bg-dark text-white border-end shadow d-flex flex-column" style="height: 100vh;">
      ...
      <ul class="list-unstyled px-3 py-3 flex-grow-1" style="overflow-y: auto;">
        ...
        <li class="mb-2">
          <a href="index.php?page=torneo_gestion&action=verificar_actas_index" class="nav-link <?= ($current_page === 'torneo_gestion' && ($_GET['action'] ?? '') === 'verificar_actas_index') ? 'active' : '' ?>">
            <i class="fas fa-qrcode me-3"></i>
            <span class="nav-text">Auditoría QR</span>
            <?php if ($actas_pendientes_count > 0): ?>
              <span class="badge bg-danger rounded-pill ms-2"><?= (int)$actas_pendientes_count ?></span>
            <?php endif; ?>
          </a>
        </li>
        <li class="mb-2">
    ...
    <div id="page-content-wrapper" class="flex-grow-1">
      ...
      <?php if ($actas_pendientes_count > 0 && in_array($user['role'], ['admin_club', 'admin_general'], true)): ?>
